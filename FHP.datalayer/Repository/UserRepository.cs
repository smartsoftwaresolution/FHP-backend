...
namespace FHP.datalayer.Repository
{
    public class UserRepository:IUserRepository
    {
       ...
       public async Task<UserDetailDto> GetUserByGovernmentId(string  governmentId,int companyId)
        ...
        public async Task UserLogOut(int userId,int companyId)
        {
            ...
            var user = await _dataContext.User.Where(s => s.Id == userId && s.LastLogOutTime!=null && s.Status != utilities.Constants.RecordStatus.Deleted).FirstOrDefaultAsync();
            if (user != null)
            {
